...
public abstract class Multifactor extends Item<Multifactor.Type>
{
	...
	public static class TOTP extends Multifactor
	{
		...
		{
			/**
			 * @param context if empty, a new secret is generated. otherwise, expects a map with the "secret" key set
			 */
			public Data enroll(User.Type user, Data context)
			{
				...
				try
				{
					if( context == null ) context = Data.map();
					
					Data info = Data.map()
						.put("secret", context.isEmpty("secret") ? base32Encode(generateSecret()) : context.asString("secret"))
