		...
		public static class Type extends Multifactor.Type
		{
			...
			protected final Data privateData(User.Type user)
			{
				try
				{
					...
					Data data = Manager.of(Vault.class).get(Manager.of(Security.class).hash(id() + "." + user.id()), this);
					if( data.isEmpty() ) return data;
					return Json.decode(data.asString());
				}
				catch(Exception e)
				{
					Manager.of(Logger.class).finest(Multifactor.class, e);
					return Data.map();
				}
			}
			...
			protected final void privateData(User.Type user, Data data)
			{
				...
				try
				{
					if( data == null ) Manager.of(Vault.class).remove(Manager.of(Security.class).hash(id() + "." + user.id()), this);
					else Manager.of(Vault.class).set(Manager.of(Security.class).hash(id() + "." + user.id()), data, this);
				}
				catch(Exception e)
				{
					Manager.of(Logger.class).finest(Multifactor.class, e);
